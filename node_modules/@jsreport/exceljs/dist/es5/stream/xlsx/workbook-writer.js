...
class WorkbookWriter {
  ...
  async waitForTemplateParse() {
    ...
    const sheets = Array.from(workbookDoc.getElementsByTagName('sheet')).map(sheetNode => {
      ...
      const sheetFileFound = waitingWorkSheets.find(w => w.ref === "xl/worksheets/sheet".concat(sheetFileId, ".xml"));
    ...
    await this.styles.parseStream(Readable.from(this.templateInfo.filesDocuments['xl/styles.xml'].originalSrc, {
      objectMode: false
    }));
    const lastWorkbookRelsId = Array.from(relsDoc.getElementsByTagName('Relationship')).reduce((acu, relNode) => {
  ...
  async commit() {
    ...
    if (this.templateInfo) {
      ...
      for (const [fileName, {
        doc: fileDoc,
        originalSrc
      }] of Object.entries(this.templateInfo.filesDocuments)) {
        let content = originalSrc;
        if (fileName === '[Content_Types].xml' && newWorksheets.length > 0) {
          const sheetNodeRef = Array.from(fileDoc.getElementsByTagName('Override')).find(node => {
            return node.getAttribute('PartName') === "/xl/worksheets/sheet".concat(this.templateInfo.lastSheetFileId, ".xml");
          });
          ...
          for (let i = 0; i < newWorksheets.length; i++) {
            ...
            newSheetNode.setAttribute('r:id', "rId".concat(this.templateInfo.lastWorkbookRelsId + i + 1));
          ...
          for (let i = 0; i < newWorksheets.length; i++) {
            ...
            newRelNode.setAttribute('Id', "rId".concat(this.templateInfo.lastWorkbookRelsId + i + 1));
            newRelNode.setAttribute('Type', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet');
            newRelNode.setAttribute('Target', "worksheets/sheet".concat(newWorksheet.id, ".xml"));
  ...
  async addWorksheetAsync(name, options) {
    ...
    return this.addWorksheet(name, _objectSpread(_objectSpread({}, options), {}, {
      async: true,
      id
    }));
  }
